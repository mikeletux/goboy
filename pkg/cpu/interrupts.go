...
import "github.com/mikeletux/goboy/pkg/bus"
...
func (c *CPU) interruptCheck(addressToJump uint16, interruptType byte) bool {
	ieRegister := c.bus.BusRead(bus.InterruptEnableRegister)
	ifRegister := c.bus.BusRead(bus.InterruptFlagIOAddr)

	if ifRegister&interruptType == interruptType &&
		ieRegister&interruptType == interruptType {

		c.pushPCToStack(addressToJump)
		c.bus.BusWrite(bus.InterruptFlagIOAddr, ifRegister & ^interruptType)
		c.Halted = false
